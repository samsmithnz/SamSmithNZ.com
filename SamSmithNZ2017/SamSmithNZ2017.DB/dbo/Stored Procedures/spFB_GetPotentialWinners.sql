...
CREATE PROCEDURE [dbo].[spFB_GetPotentialWinners] 
	@year_code smallint,
	@week_code smallint--,	@current_date datetime
AS
SET NOCOUNT ON
...
SELECT record_id--, game_time, dateadd(hh,+2,getdate())
FROM FBWeekTemplate
WHERE year_code = @year_code
and week_code = @week_code
--and game_time >= dateadd(hh,-3,getdate())--used to be -3, but since it is hosted in mountain time, make it -1
...
*/
	...
	UNION
	SELECT 'AA', player_name + ' - ' + convert(varchar(3),wins_this_week+1) + ' wins', wins_this_week+1
	FROM #tmp_result_multiple_games mg
	INNER JOIN FBPlayer p ON mg.player_code = p.player_code and p.year_code = @year_code
	INNER JOIN #tmp_results_base rb ON p.player_code = rb.player_code
	...
	--AB
	UNION
	SELECT 'AB' as code, player_name + ' - ' + convert(varchar(3),wins_this_week+1) + ' wins' as [text], wins_this_week+1
	FROM #tmp_result_multiple_games mg
	INNER JOIN FBPlayer p ON mg.player_code = p.player_code and p.year_code = @year_code
	INNER JOIN #tmp_results_base rb ON p.player_code = rb.player_code
	WHERE pick1 = 'A' and pick2 = 'A'
	UNION
	SELECT 'AB', player_name + ' - ' + convert(varchar(3),wins_this_week+2) + ' wins', wins_this_week+2
	FROM #tmp_result_multiple_games mg
	INNER JOIN FBPlayer p ON mg.player_code = p.player_code and p.year_code = @year_code
	INNER JOIN #tmp_results_base rb ON p.player_code = rb.player_code
	WHERE pick1 = 'A' and pick2 = 'B'
	...
	WHERE pick1 = 'B' and pick2 = 'A'
	UNION
	SELECT 'AB', player_name + ' - ' + convert(varchar(3),wins_this_week+1) + ' wins', wins_this_week+1
	FROM #tmp_result_multiple_games mg
	INNER JOIN FBPlayer p ON mg.player_code = p.player_code and p.year_code = @year_code
	INNER JOIN #tmp_results_base rb ON p.player_code = rb.player_code
	WHERE pick1 = 'B' and pick2 = 'B'
	...
	--BA
	UNION
	SELECT 'BA' as code, player_name + ' - ' + convert(varchar(3),wins_this_week+1) + ' wins' as [text], wins_this_week+1
	FROM #tmp_result_multiple_games mg
	INNER JOIN FBPlayer p ON mg.player_code = p.player_code and p.year_code = @year_code
	INNER JOIN #tmp_results_base rb ON p.player_code = rb.player_code
	WHERE pick1 = 'A' and pick2 = 'A'
	...
	WHERE pick1 = 'A' and pick2 = 'B'
	UNION
	SELECT 'BA', player_name + ' - ' + convert(varchar(3),wins_this_week+2) + ' wins', wins_this_week+2
	FROM #tmp_result_multiple_games mg
	INNER JOIN FBPlayer p ON mg.player_code = p.player_code and p.year_code = @year_code
	INNER JOIN #tmp_results_base rb ON p.player_code = rb.player_code
	WHERE pick1 = 'B' and pick2 = 'A'
	UNION
	SELECT 'BA', player_name + ' - ' + convert(varchar(3),wins_this_week+1) + ' wins', wins_this_week+1
	FROM #tmp_result_multiple_games mg
	INNER JOIN FBPlayer p ON mg.player_code = p.player_code and p.year_code = @year_code
	INNER JOIN #tmp_results_base rb ON p.player_code = rb.player_code
	WHERE pick1 = 'B' and pick2 = 'B'
	...
	WHERE pick1 = 'A' and pick2 = 'A'
	UNION
	SELECT 'BB', player_name + ' - ' + convert(varchar(3),wins_this_week+1) + ' wins', wins_this_week+1
	FROM #tmp_result_multiple_games mg
	INNER JOIN FBPlayer p ON mg.player_code = p.player_code and p.year_code = @year_code
	INNER JOIN #tmp_results_base rb ON p.player_code = rb.player_code
	WHERE pick1 = 'A' and pick2 = 'B'
	...
	WHERE pick1 = 'B' and pick2 = 'A'
	UNION
	SELECT 'BB', player_name + ' - ' + convert(varchar(3),wins_this_week+2) + ' wins', wins_this_week+2
	FROM #tmp_result_multiple_games mg
	INNER JOIN FBPlayer p ON mg.player_code = p.player_code and p.year_code = @year_code
	INNER JOIN #tmp_results_base rb ON p.player_code = rb.player_code
	WHERE pick1 = 'B' and pick2 = 'B'
...
SELECT '1' as code, 
	...
	+ convert(varchar(5),@fav_wins_count) + ' people win with ' 
	+ convert(varchar(5),@fav_wins) + ' wins each.' as [text]
...
SELECT '3', 
	...
	+ convert(varchar(5),@spread * -1) + ' or more, then ' 
	+ convert(varchar(5),@underdog_wins_count) + ' people win with ' 
	+ convert(varchar(5),@underdog_wins) + ' wins each.'
