CREATE PROCEDURE [dbo].[spFB_GetPicks]
	@year_code smallint,
	@week_code smallint,
...
CREATE TABLE #tmp_counts (record_id uniqueidentifier, fav_count decimal(10,4))

INSERT INTO #tmp_counts 
SELECT w.record_id, sum(w.fav_team_picked)
FROM FBWeek w
WHERE w.year_code = @year_code and w.week_code = @week_code and fav_team_picked >= 0 
...
SELECT w.record_id, w.week_code, w.player_code, wt.home_team_code, t1.image_name as home_image_name, 
	...
	wt.fav_team_code, wt.game_time, wt.spread, w.fav_team_picked, w.last_updated,
	wt.home_team_result, wt.away_team_result, wt.fav_team_won_game, w.won_pick, convert(decimal(10,4),isnull((c.fav_count/@week_player_total),0)) as fav_picked_count
FROM FBWeek w
