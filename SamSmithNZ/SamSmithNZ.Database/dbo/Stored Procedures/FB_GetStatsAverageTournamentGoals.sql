...
BEGIN
	...
	SELECT t.tournament_code AS TournamentCode,
		...
		CONVERT(DECIMAL(10,2),SUM(g.team_1_normal_time_score) + SUM(ISNULL(g.team_1_extra_time_score,0)) + SUM(g.team_2_normal_time_score) + SUM(ISNULL(g.team_2_extra_time_score,0))) / 
			CONVERT(DECIMAL(10,2),(SELECT COUNT(g4.game_code) FROM wc_game g4 WHERE g4.tournament_code = t.tournament_code AND g4.game_time <= @CurrentTime)) AS AverageGoalsPerGame
